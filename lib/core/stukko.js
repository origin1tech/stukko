	...
	this.options = {
		...
		assets: {                                                       // if defined manages assets, compiles less, concat, minify etc.
			...
			link: {                                                     // see https://github.com/klei/gulp-inject for full options including starttag, endtag and transform if not using .html files. set to false to disable linking.
				ignorePath: ['/web/public']                             // include any static route prefixes here so they are stripped from your assets.
		...
		middleware: {
			...
			favicon: { use: 'serve-favicon', options: '/web/public/img/favicon.ico' },
 ...
 * Initialize Stukko.
 */
Stukko.prototype.init = function init() {
	...
	if(this.cli && this.cmd !== 'start'){
		...
		log.debug('Configuration complete start the server.');
...
Stukko.prototype.listen = function listen(host, port, cb) {
	...
	server.on('listening', function () {
		...
		// TODO: need to make sure this is working properly.
