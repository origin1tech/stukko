        ...
        Promise.all(promises).then(
            function () {
                ...
                if(!self.start)
                    ...
                    $$LOG.console.info('\nSuccesfully processed (' + promises.length + ') seeds.\n');
            }
        );
    }
};
 ...
 * @param {string|array} modules - modules to process csv string or array..
 * @param {function} [cb] - callback to call on completion.
 */
Stukko.prototype.npm = function npm(cmd, modules, cb) {

	var self = this,
		npm = utils.npm.call(this),
        npmConfig = _.clone(this.flags);
...
Stukko.prototype.git = function git(cmd, args) {
    ...
    if(gitCmd !== 'generic' && validCmds.indexOf(gitCmd) === -1){
