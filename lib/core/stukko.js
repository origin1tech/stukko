...
function Stukko(options, config) {
	...
	this.diag = diag;                                                   // calling .get() returns current diagnostics.
	...
	this.options = {
		...
		assets: {                                                       // if defined manages assets, compiles less, concat, minify etc.
			...
			ignorePath: ['/web/public', '/manage/public'],              // include any static route prefixes here so they are stripped from your assets.
				starttag: '<!-- inject:{{ext}} -->',
				endtag: '<!-- endinject -->'
			},
			...
			mixin: {
				src: ['./web/assets/mixin/**/*.css', './web/assets/mixin/**/*.js', './web/assets/mixin/**/*.css', './manage/assets/mixin/**/*.js'],
				concat: ['mixins.css', 'mixins.js', 'mixins.css', 'mixins.js'],
		...
		middleware: {
			...
			static: { use: express.static, options: '/web/public' },
			partials: { use: express.static, options: '/web/public/partials' },
...
Stukko.prototype.create = function create() {
	...
	utils.io.copy(structure, appPath, function (err) {
		...
		if(!err){
			...
			utils.io.write(self.cwd + '/package.json', JSON.stringify(pkg, null, 4));
...
Stukko.prototype.info = function info(key) {
	...
	proc = this.diag.get(null, true).process;
	oper = this.diag.get(null, true, ['cpus']).os;
