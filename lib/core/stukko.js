...
function Stukko(options, config) {
	...
	this.directory = path.basename(cwd);
	...
	this.options = {
		...
		middleware: {
			...
			session: { use: 'express-session', options: { secret: '123456', key: 'sid' }, require: true, enabled: true },
			...
			utils.npm(packages, appPath, function (err) {
				if(err)
					throw new Error('Stukko was unable to auto install npm packages.' +
						...
						'\nFor Windows users you may need to specify the --msvs_version=2012 or whatever version of visual studio you may have installed.' +
						'\nex: stukko create appName --msvs_version=2010.');
				// update stukko version.
				pkg.dependencies.stukko = self.pkg.version;
				// finally write the package.
