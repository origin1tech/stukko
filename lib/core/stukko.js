...
Stukko.prototype.init = function init(cb) {
	...
	var self = this,
		...
		if(!this.pkgapp){
			...
			ver = this.pkgapp.dependencies.stukko;
			// if git exists as version filter version.
			if(/(https|git|ssh)/.test(ver))
				ver = ver.match(/[0-9]\.[0-9]\.[0-9]/g)[0];
			if(!/(https|git)/.test(this.pkgapp.dependencies.stukko)){
...
Stukko.prototype.listen = function listen(host, port, cb) {
	...
	server.on('listening', function () {
		...
		log.debug('Configuring listeners for process signals.');
...
Stukko.prototype.create = function create() {
	...
	utils.io.copy(structure, appPath, function (err) {
		...
		packages.push('https://github.com/origin1tech/stukko/archive/' + self.pkg.version + '.tar.gz');
		pkg.dependencies.stukko = 'https://github.com/origin1tech/stukko/archive/' + self.pkg.version + '.tar.gz';
		if(!err){
