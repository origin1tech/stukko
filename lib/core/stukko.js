...
Stukko.prototype.restore = function restore(bkupPath) {
	...
	prompt.get({
		...
		if(!/yes/g.test(result.confirm)){
            ...
            // compares dependencies so as to use the latest.
...
Stukko.prototype.help = function help() {
    ...
    help += newline + spacer + 'checkout:  '.green + ' checkout master or release from source repository.\n';
...
Stukko.prototype.npm = function (cmd, modules, cb) {
	...
	function removeAll() {
	  ...
	  if(!modules.length)
        return $LOG.warn('Nothing to update exiting npm...');
