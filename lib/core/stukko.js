...
Stukko.prototype.seed = function seed() {
    ...
    if(this.options.env === 'production')
       return $$LOG.console.warn('Seeding is disabled in production mode. Use --force or -f to force seeding.');
    ...
    if(this.modules.seeds && Object.keys(this.modules.seeds).length) {
        ...
        _.forEach(this.modules.seeds, function(v,k) {
            ...
            if(_.isFunction(v)){
                ...
                if(module === 'mongoose' && self.db.models && Object.keys(self.db.models).length){
                    promise = new Promise(_.bind(v, self, self.db, self.db.schema, self.db.model, chance));
                }
        ...
        Promise.all(promises).then(
            function () {
                console.log('\n');
                if(!self.start)
                    self.exit('Succesfully processed (' + promises.length + ') seeds actions.\n');
                else
                    $$LOG.console.info('Succesfully processed (' + promises.length + ') seeds actions.\n');
            }
        );
    }
};
