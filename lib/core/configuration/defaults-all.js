...
module.exports = function () {
	...
	return  {
		...
		statusErrors: {                                                 // when true handlers are added to the stack to catch errors that are thrown and catch 404 when pages not found.
            404: true,
            500: true
        },
        catchAll: false,                                                // when true a catch all route that resolves layout is used.
		...
		middleware: {
			...
			inject: {
				use: '{{internal}}/middleware/inject'                   // {{internal}} is used by Stukko to map to root of itself NOT the application.
			}
		},
		assets: {                                                       // if defined manages assets, compiles less, concat, minify etc.
			...
			preprocess: {
                ...
                watch: ['./web/assets/preprocess/**/*.less', './web/assets/preprocess/**/*.scss']      // designates which dirs to watch since src is only root preprocessed files. set to false to disable.
			},
            bundle: {
                framework: {
                    exclude: [],
                    src: ['./web/assets/framework/**/*.js'],
                    concat: ['app.js'],                                 // the concatenated output file name.
                    dest: ['./web/public/js'],
                    minify: false,
                    strategy: 'concat',                                 // options are "concat" or "browserify".
			        watch: true,                                        // when true changes are rebundled.
                    options: undefined                                  // object of options for browserify or {newLine: ';'} for concat.
                },
            },
            link: {                                                     // see https://github.com/klei/gulp-inject for full options including starttag, endtag & transform if not using.
                common: {                                               // links common support libs for your application.
                    ...
                    files: [
                        './web/public/css/**/*.css',                    // files are created in order if you wish you can specify them individually.
                        './web/public/js/**/*.js'
                    ],
