...
function Configure(opts, config) {
	...
	function modules() {
		...
		_.forEach(self.options.modules, function (v,k) {
			...
			// make models global.
			if(k === 'models' && self.options.db.load){
				_.forEach(keys, function(key) {
					var globalKey = helpers.stringToCase(key, self.options.db.modelCase);
					...
					if(_.isFunction(module[key]) && !Object.keys(module[key]).length){
                        // if db specified pass in db connect for defining models.
                        var tmpModule, modelName;
                        ...
                        try{
                            if(self.options.db){
                                ...
                                if(self.options.db.module === 'sequelize'){
                                    tmpModule = module[key].call(self, self.db.connection, self.db.client);
                                    modelName = tmpModule.name || modelName;
                                }
                                if(self.options.db.module === 'mongoose'){
                                    tmpModule = module[key].call(self, self.db.Schema, self.db.model);
                                    modelName = tmpModule.modelName || modelName;
                                }
                                if(self.options.db.module === 'mongodb'){
                                    tmpModule = module[key].call(self, self.db.connection);
                                    modelName = tmpModule.collectionName || modelName;
                                }
                                ...
                                if(tmpModule){
                                    module[modelName] = tmpModule;
                                    GLOBAL[modelName] = tmpModule;
                                }
                        ...
                        // just add plain object for convenience.
						GLOBAL[globalKey] = module[key];
					}
				});
                // post model creation
                if(self.options.db.module === 'sequelize'){
                   // iterate any associations.
                    Object.keys(module).forEach(function(modelName) {
                        if ('associate' in module[modelName]) {
                            module[modelName].associate(module);
                        }
                    });
                }
			}
	...
	******************************************************/
    ...
    this.on('command', function () {
        ...
        if(!this.cmd && !isUserCommand) return;
            //throw new Error('Stukko could not process command ' + this.ocmd + '. Try stukko help for list of commands.');
