...
function Configure(opts, config) {
	...
	function modules() {
        ...
        function sequelizeAssociations(models) {
            Object.keys(models).forEach(function(modelName) {
                if ('associate' in models[modelName].options) {
                    $LOG.debug('Creating association for model ' + modelName);
        ...
        function seed() {
            ...
            if(self.options.env === 'development' && Object.keys(self.modules.seeds).length) {

                // iterate each seed and update table.
                _.forEach(self.modules.seeds, function(v,k) {
                    if(_.isFunction(v)){
                        if(self.options.db.module === 'sequelize')
                            v.call(self, self.db.connection, self.db.client, helpers.chance);

                        if(self.options.db.module === 'mongoose')
                            v.call(self, self.db.Schema, self.db.model, helpers.chance);

                        if(self.options.db.module === 'mongodb')
                            v.call(self, self.db.connection, helpers.chance);
                ...
                if(dbConf.module === 'sequelize'){
                    ...
                    _.forEach(keys, function(key) {
                        ...
                        if(_.isFunction(module[key])){
                            ...
                            if(self.options.db.module === 'dirty'){
                                tmpModel = module[key].call(self, self.db.connection);
                                modelName = tmpModel.name || modelName;
                            }
