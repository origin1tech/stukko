...
function Configure(opts, config) {
	...
	function loggers() {
        ...
        // if start not requested just emit
        // ready, no need to continue initializing.
        if(!self.start)
	...
	function assets() {
		...
		if(!self.options.assets || self.debug)
	...
	******************************************************/
    ...
    this.on('command', function () {
        ...
        // call command.
        if(_.contains(['install', 'update', 'uninstall'], this.cmd))
            this.npm(this.cmd);
        else
            this[this.cmd]();
    });
