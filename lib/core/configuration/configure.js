...
function Configure(opts, config) {
	...
	function routes(){
        ...
        var generatorName = (routeOptions.generator || 'generator').replace('.js', ''),
                ...
                _.forEach(modelKeys, function (k) {
                    ...
                    if(routeOptions.rest){
                        ...
                        _.forEach(restPaths, function (rest, key) {
                            var pols = lookupPolicy(k, generatorMap[rest.action].name),
                                route;                         
                            pols = pols.concat(generatorActions[rest.action]);
                        ...
                        _.forEach(generatorMap, function (action, key) {
                            _.forEach(action.methods, function(m) {
                                ...
                                if(action.params.length){
                                    ...
                                    var route, pols;
                                    pols = lookupPolicy(k, action.name);
                                    pols = pols.concat(generatorActions[action.name])
