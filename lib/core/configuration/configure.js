...
function Configure(opts, config) {
	...
	function options() {
        ...
        // LEGACY: Ensure options.src instead of options.src.
        self.options = self.options || {};
        self.options.src = self.options.src || _.clone(self.options.assets || {});
		...
		if(self.start) {
            ...
            self.options.src.bundle = self.options.src.bundle || {};

            // LEGACY: convert assets.framework to assets.bundle.framework;
            if(self.options.src && self.options.src.framework && !self.options.src.bundle.framework){
                    self.options.src.bundle.framework = self.options.src.framework;
                    delete self.options.src.framework;
                if(_.isEmpty(self.options.src.bundle))
                    delete self.options.src.bundle;
            }

            // LEGACY: add watching to each key other than like.
            if(self.options.src && self.options.src.watch !== undefined){
                _.forEach(self.options.src, function (v,k) {
                    if(k !== 'link' && k !== 'bundle' && k !== 'preprocess')
                        self.options.src[k].watch = self.options.src.watch;
                    else if(k === 'bundle'){
                        _.forEach(self.options.src.bundle, function (v,k) {
                            self.options.src.bundle[k].watch = self.options.src.watch;
                        });
                    }
                });
                delete self.options.src.watch;
            }
        ...
        if(self.options.modules.services) {
            ...
            _.forEach(tmpServices, function (v,k) {
                ...
                if(_.isPlainObject(v)){
                    ...
                    self.modules.services[serviceName] = v;
                }
            });
	...
	function routes(){
        ...
        if(!_.contains(self.routers.default.routes, 'get /')){
            var route, pols
            pols = lookupPolicy();
            pols.push(function (req, res, next){
                res.render(layout);
        ...
        if(routeOptions.models !== false){
            ...
            if(generatorMap) {
                ...
                _.forEach(modelKeys, function (k) {
                    ...
                    // get defined config rest actions.
                    //restActions = routeOptions.restActions || routeOptions.generatorActions;
                    ...
                    if(routeOptions.rest !== false){
                        ...
                        //    restPaths = [];
                        //
                        //findAll = restActions['findAll'];
                        //find = restActions['find'];
                        //create = restActions['create'];
                        //update = restActions['update'];
                        //destroy = restActions['destroy'];
                        //
        ...
        if(self.env === 'development') {
            ...
            app.get('/stukko', function (req, res, next) {
                ...
                try{
                    ...
                    addKey('routes:');
                    // Todo: refactor to support multiple routers.
                    _.forEach(self.routers, function (v,k) {
	...
	function src() {
		...
		if(!self.options.src || self.options.src.enabled === false)
            return self.emit('src:ready');
    ...
    function listening() {
        ...
        $$LOG.console.info('Build Source: ' + (this.options.src && this.options.src.enabled !== false ? true : false).toString() );
	...
	******************************************************/
    ...
    this.on('routes:ready', src);
    this.on('src:ready', function() { this.emit('ready'); });
