...
function Configure(opts, config) {
	...
	function modules() {
        ...
        /**
         * @todo - wire up to support global schema options before attach
         * @author - Origin1 Tehcnologies <origin1tech@gmail.com>
         * @date - 2/10/2015
         */
        function modelHook() {
            ...
            // get hooks.
            if(self.options.hooks && self.options.hooks.beforeModels)
                modelHooks = hooks.call(self);
            
            // if model and has hooks
            if(modelHooks && modelHooks.beforeModels && arguments[0])
                modelHooks.beforeModels.apply(self, arguments);            
        }
    ...
    function listening() {
        ...
        $$LOG.console.info('Build Assets: ' + (this.options.assets && this.options.assets.enabled !== false ? true : false).toString() );
