	...
	function connect(options) {
		...
		if(dbconf.connect) {
			if (dbconf.module === 'dirty' || dbconf.module === 'redis') {
				...
				if(dbconf.module === 'sequelize'){
					db.connection = new db.client(dbconf.options.database, dbconf.options.username, dbconf.options.password, dbconf.options);
					db.connection
						.authenticate()
						.complete(function(err) {
							if (!!err) {
								...
								db.connected = true;
                                // TODO: test disconnect for all dialects only mysql currently working.
                                db.destroy = function () {};
                                if(_.contains(['mysql'], dbconf.options.dialect))
