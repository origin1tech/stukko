...
function Db() {
  ...
  function client(moduleName) {
    ...
    reqPath = moduleName === 'stukko-dirty' ? moduleName : p.join(self.cwd, '/node_modules/', moduleName);
  ...
  function connect(options) {
    ...
    if (dbconf.module === 'mongoose') {
      ...
      args.push(function(err) {
        ...
        // monkey patch ensure 4.4.x still disconnects in same manner.
        db.destroy = function disconnectMongoose(fn) {
          let error, count = 0;
          db.connection.connections.forEach(function(conn){
            ...
            db.connection
              ...
              .complete(function(err) {
                if (err) {
                  ...
                  db.connected = true;
                  db.destroy = function() {};
                  $$DEBUG('Sequelize connection to database ' + dbconf.options.storage + ' using SQL engine/dialect ' + dbconf.options.dialect + ' has been established successfully.');
                  done();
                }
              });
          } else {
            ...
            db.connection
              ...
              .complete(function(err) {
                if (!!err) {
                  ...
                  $$DEBUG('Sequelize connection to database ' + dbconf.options.database + ' using SQL engine/dialect ' + dbconf.options.dialect + ' has been established successfully.');
                  done();
                }
              });
          }
