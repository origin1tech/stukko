	...
	return {
		...
		options: function options() {
			...
			if(self.options.middleware.i18n.enabled) {
				if(!self.options.middleware.i18n.options){
					self.options.middleware.i18n.options = {
						locales:['en', 'es'],
						directory: self.cwd + '/server/locales',
						defaultLocale: 'en'
					};
					saveConfig = true;
				}
			...
			//if(self.flags.save === true || !hasDefault || saveConfig){
				...
				utils.io.write(self.cwd + '/server/configuration/' + config + '.json', JSON.stringify(self.options, null, 4));
			//}
		...
		middleware: function middleware() {
			...
			if(merged.session.enabled || merged.session.enabled === undefined){
				...
				var MemoryStore, RedisStore, MongoStore,
					mongoDefaults, redisDefaults,
				...
				if(sessionOptions.name){
					if(sessionOptions.name === 'memory'){
						...
						self.sessionStore = new RedisStore(_.extend(redisDefaults, sessionOptions.storeOptions));
					} else if (/mongo/gi.test(sessionOptions.name)){
						MongoStore = require('connect-mongo')(expressSession);
						self.sessionStore = new MongoStore(_.extend(mongoDefaults, sessionOptions.storeOptions));
					} else {
