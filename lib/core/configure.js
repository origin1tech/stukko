...
function Configure(opts, config) {
	...
	return {
		...
		loggers: function loggers() {
			...
			$LOG.info('Loading configuration ' + self.config);
			self.emit('loggers:completed');
		},
		...
		middleware: function middleware() {
			...
			_.forEach(defKeys, function (k) {
				...
				function done(opts) {
					...
					self.emit('middleware:completed');
				}
