...
function Configure(opts, config) {
	...
	return {
		...
		management: function management() {
			...
			/* middleware
			****************************************/
			if(self.options.middleware.logger.enabled)
			...
			router.use(self.express.static(self.cwd + '/manage/views'));

			/* routes
			****************************************/

			// common function to simply render the layout.
			function renderCommon(req, res, next) {
				self.optionsOriginal.cwd = self.cwd;
				var diag = self.diag.get(),
					model;
				diag.server.packets = [];
				model = {
				...
				res.render('manage', { config: model.config, pkg: model.pkg, diag: model.diag });
			}

			router.get('/', function (req, res) { res.redirect('/manage/dashboard');});
			router.get('/dashboard', renderCommon);
			router.get('/editor', renderCommon);
			router.get('/wizard', renderCommon);

            router.get('/dashboard/refresh', function(req, res, next) {
                var model = {
	                    diag: self.diag.get()
	                },
                    interval = req.query.interval || 5,
	                ts = req.query.ts || Date.now();
	            model.diag.server.packets = self.network.packets(interval, ts);
                res.json(model);
            });

			router.get('/dashboard/network', function(req, res, next) {
				res.json({});
			});

			router.get('/editor/show', function (req, res, next) {
				var file = self.cwd + '/server/configuration/development.json';
				if(req.query && req.query.file)
					file = self.cwd + req.query.file;
						utils.io.exists(file, function (exists) {
					if(!exists)
						return res.json(404, 'The requested file ' + file + ' could not be found.');
					utils.io.read(file, null, function(err, data) {
						if(err){
							res.json(500, 'Unknown error loading file ' + file + '.');
						} else {
			...
			router.post('/editor/update', function (req, res, next) {
