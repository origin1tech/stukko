...
function getExportFiles(bundleName) {
    var found = [],
        result = {},
    ...
    if(!bundle)
        return;
    groupBy = bundle.imports;
    // if not groupBy generate
    // imports is disabled.
    if(!groupBy)
        return;
    // get blog source.
    globs = bundle.src;
    // ensure globs/groupBy are arrays.
    if(!(globs instanceof Array))
        globs = [globs];
    if(!(groupBy instanceof Array))
        groupBy = [groupBy];
    // using globs find files.
    found = globby.sync(globs);
    if(!groupBy.length){
        result = found;
    } else {
      groupBy.forEach(function (g) {
        ...
        found.forEach(function (f) {
            result[g] = result[g] || [];
            var seg = f.split('/'),
                ignore = seg.pop().charAt(0) === '_';
            if(f.indexOf(g) !== -1 && f.indexOf('index.js') === -1 && !ignore){
                var relPath = f.toLowerCase().split(g)[1];
                if(relPath)
                result[g].push('.' + relPath);
            }
        });
      });
    }
...
deploy = assets ? assets.deploy : undefined;

// make sure we have assets to process.
if(!assets || assets.enabled === false)
...
gulp.task('bundle', ['clean'], function () {
    ...
    keys.forEach(function(k) {
        ...
        if(bundle.strategy === 'browserify'){
            ...
            if(!plugins.browserify)
                throw new Error('Plugin not installed, run "npm install gulp-browserify" from prompt.');
        ...
        if(bundle.strategy === 'es6'){

            if(!plugins.babel)
                throw new Error('Plugin not installed, run "npm install gulp-babel" from prompt.');

            if(bundle.minify){

                task = gulp.src(normalizeArray(bundle.src))
                    .pipe(plugins.sourcemaps.init())
                    .pipe(plugins.babel(bundle.options))
                    .pipe(plugins.uglify())
