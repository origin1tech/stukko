...
function link () {
	...
	var layout = '.' + config.express.views + '/' +
		 ...
		 appSrc = [],
		 manageSrc = [],
		 manageConf = assets.link,
		 appLink,
		 manageLink;
	// add exclude for errors we don't want to injet it in layout.
	sources.push('!./web/public/css/errors.css');
	sources.forEach(function(s) {
		if(s.indexOf('manage') === -1)
			appSrc.push(s);
		else
			manageSrc.push(s);
	});
	appLink = gulp.src(layout)
		.pipe(inject(gulp.src(appSrc, {read: false}), assets.link))
		.pipe(gulp.dest('.' + config.express.views));
	// since manage uses own router
	// static resources need prefix.
...
gulp.task('watch', ['backup', 'clean', 'framework'], watch);
