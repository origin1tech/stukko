...
gulp.task('exports', function(cb){

    var parserConf, parserKeys;

    if(loaded || !parser)
        return cb();

    parserConf = config.parsers;

    if(config.parser && !parserConf)
        parserConf = { 'default': config.parser };

    // key array of keys.
    parserKeys = Object.keys(parserConf || {});


    if(!parserKeys || !parserKeys.length)
        return cb();

    parserKeys.forEach(function(k) {

        var _parser = parserConf[k];

        // should never hit this but..
        if(!_parser)
            return;

        parser.genExports(_parser);
...
gulp.task('bundle', ['exports', 'clean'], function () {
    ...
    keys.forEach(function(k) {
        ...
        // systemjs bundle.
        if(bundle.strategy === 'systemjs' && !loaded) {

            //var Builder = tryRequire('systemjs-builder');
            var Builder = tryRequire('jspm').Builder;
            var through = require('through2');
            var jspm = pkg.jspm;

            if(!Builder)
                throw new PluginError('Gulp-Build',
                    'Plugin not installed, run "npm install jspm --save" from terminal.');
            ...
            var buildType = 'build';
            var configFile = jspm.configFile || 'config.js' || 'jspm.config.js';
            var baseURL = p.join('./', jspm.directories.baseURL);
            ...
            task = gulp.src(src)
                .pipe(through.obj(function (file, enc, cb) {
                    ...
                    builder.loadConfig(configFile)
                        .then(function() {
                            builder.config({baseURL: baseURL});
                            // uncomment if using systemjs-builder
                            // instead of require('jspm').Builder();
...
gulp.task('link', ['clean', 'bundle'], function () {
    ...
    keys.forEach(function(k) {
        ...
        // if note rename generate by link.to value.
        rename = link.rename;
        if(!rename){
            rename = p.basename(link.to);
            var exp = new RegExp('.' + config.express['view engine']);
            rename = rename.replace(exp, '');
        }
        ...
        task = gulp.src(link.to)
            ...
            // gulp-if acting funny always rename by passing
            // parsed value if link.rename doesn't exist.
